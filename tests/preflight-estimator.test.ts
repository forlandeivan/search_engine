...
const tokenModel = { consumptionUnit: "TOKENS_1K", creditsPerUnit: 1000 } as any;
const minuteModel = { consumptionUnit: "MINUTES", creditsPerUnit: 300 } as any;
...
describe("Preflight estimator", () => {
  it("LLM adds max output tokens and rounds up to 1k blocks", () => {
    ...
    expect(result.estimatedCreditsCents).toBe(3000);
  });
  ...
  it("Embeddings uses input tokens only", () => {
    ...
    expect(result.estimatedCreditsCents).toBe(1000);
  });
  ...
  it("ASR converts seconds to minutes with ceil", () => {
    ...
    expect(result.estimatedCreditsCents).toBe(600);
  });
});
