...
describe("SpeechProviderService - ASR Provider Management", () => {
  ...
  describe("unicaAsrConfigSchema", () => {
    ...
    it("should reject timeouts outside range", () => {
      const tooShortTimeout = {
        baseUrl: "https://test.example.com/api",
        workspaceId: "TEST",
        timeoutMs: 30000, // Less than 60000 minimum
      };

      const result = unicaAsrConfigSchema.safeParse(tooShortTimeout);
      expect(result.success).toBe(false);

      const tooLongTimeout = {
        baseUrl: "https://test.example.com/api",
        workspaceId: "TEST",
        timeoutMs: 8000000, // More than 7200000 maximum
      };

      const result2 = unicaAsrConfigSchema.safeParse(tooLongTimeout);
