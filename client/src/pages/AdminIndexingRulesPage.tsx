...
import {
  ...
  MAX_CHUNK_SIZE as KB_MAX_CHUNK_SIZE,
  MIN_CHUNK_SIZE as KB_MIN_CHUNK_SIZE,
    ...
    <div className="p-6 space-y-4">
      ...
      <Tabs defaultValue="skills" className="space-y-4">
        ...
        <TabsContent value="knowledge-bases" className="space-y-4">
          <KnowledgeBaseIndexingPolicyTab />
        </TabsContent>
      </Tabs>
    </div>
  );
...
function KnowledgeBaseIndexingPolicyTab() {
  ...
  const onSubmit = form.handleSubmit(async (values) => {
    try {
      ...
      setBaseline(updated);
      form.reset(updated);
      toast({ title: "Сохранено", description: "Политика индексации баз знаний обновлена" });
    } catch (err: unknown) {
      const apiErr = err as ApiError;
      const details = (apiErr?.details ?? {}) as Record<string, unknown>;
      const message =
        apiErr instanceof ApiError
          ...
          : err instanceof Error
            ? err.message
            : "Не удалось сохранить политику индексации";
      if (details?.field === "embeddings_provider") {
        form.setError("embeddingsProvider", { message });
      }
      if (details?.field === "embeddings_model") {
  ...
  if (isLoading) {
    return (
      <div className="p-6 text-muted-foreground flex items-center gap-2">
        <Loader2 className="h-4 w-4 animate-spin" />
        Загружаем политику индексации...
      </div>
    );
  }

  if (isError) {
    const message = (error as Error)?.message ?? "Не удалось загрузить политику индексации";
    return (
      <div className="p-6">
        <Alert variant="destructive">
          <AlertTitle>Ошибка загрузки</AlertTitle>
  ...
  return (
    <Card>
      <CardHeader className="flex flex-col gap-2">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div>
            <CardTitle>Профиль индексации баз знаний</CardTitle>
          ...
          <form className="space-y-6" onSubmit={onSubmit}>
            <section className="space-y-4">
              <div>
                <h3 className="text-lg font-semibold">Embeddings</h3>
                <p className="text-sm text-muted-foreground">
                  Провайдер и модель, используемые для векторизации документов баз знаний.
                </p>
              </div>
              <div className="grid md:grid-cols-2 gap-4">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel htmlFor="kb-indexing-embeddings-provider">Провайдер эмбеддингов</FormLabel>
                      <FormControl>
                        <Select
                          disabled={providerFieldDisabled}
                          value={field.value}
                        ...
                        >
                          <SelectTrigger id="kb-indexing-embeddings-provider">
                            <SelectValue
                              placeholder={
                                providersQuery.isLoading
                                  ? "Загружаем провайдеры..."
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel htmlFor="kb-indexing-embeddings-model">Модель эмбеддингов</FormLabel>
                      <FormControl>
                        {supportsModelSelection && modelOptions.length > 0 ? (
                          <Select
                            value={field.value ?? ""}
                          ...
                          >
                            <SelectTrigger id="kb-indexing-embeddings-model">
                              <SelectValue
                                placeholder={
                                  modelsLoading
                                    ? "Загружаем модели..."
                          ...
                          <Input
                            id="kb-indexing-embeddings-model"
                            placeholder="text-embedding-3-small"
                            disabled={disableInputs || modelsLoading || !supportsModelSelection}
                            value={field.value ?? ""}
                            onChange={(event) => {
            ...
            <section className="space-y-4">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel htmlFor="kb-indexing-chunk-size">Размер чанка</FormLabel>
                      <FormControl>
                        <Input
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel htmlFor="kb-indexing-chunk-overlap">Перекрытие чанков</FormLabel>
                      <FormControl>
                        <Input
                          id="kb-indexing-chunk-overlap"
                          type="number"
                          min={0}
                          max={overlapMax}
                          step={1}
