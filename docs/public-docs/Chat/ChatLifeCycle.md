...
Если навык RAG (isRagSkill): вызывается callRagForSkillChat. В payload идёт расширенный запрос (q), построенный из истории разговора и текущего сообщения пользователя (chat-rag.ts). Используется multi-turn RAG: берутся последние N сообщений (по умолчанию 6, настраивается в навыке) и объединяются с текущим запросом для улучшения качества поиска. История ограничена по количеству символов (по умолчанию 4000, настраивается в навыке). История также передаётся в LLM completion, чтобы LLM видел предыдущие сообщения в контексте диалога.
