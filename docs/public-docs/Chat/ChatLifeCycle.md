...
Если навык RAG (isRagSkill): вызывается callRagForSkillChat. В payload идёт расширенный запрос (q), построенный из истории разговора и текущего сообщения пользователя (chat-rag.ts). Используется multi-turn RAG: берутся последние 5 сообщений (максимум 2000 символов) и объединяются с текущим запросом для улучшения качества поиска.
