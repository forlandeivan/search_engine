...
export async function ensureDatabaseSchema(): Promise<void> {
  try {
    ...
    // ASR executions table for tracking speech-to-text operations
    await db.execute(sql`
    ...
    `);
    try {
      await db.execute(sql`CREATE INDEX CONCURRENTLY IF NOT EXISTS asr_executions_created_at_idx ON asr_executions (created_at DESC)`);
    } catch (error) {
      swallowPgError(error, ["42710", "42P07"]);
    }
    try {
    ...
    try {
      await db.execute(sql`CREATE INDEX CONCURRENTLY IF NOT EXISTS asr_executions_status_idx ON asr_executions (status, created_at DESC)`);
    } catch (error) {
      swallowPgError(error, ["42710", "42P07"]);
    }
