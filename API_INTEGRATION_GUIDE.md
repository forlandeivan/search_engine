...
# API Поискового Движка — Интеграция c базами знаний

## Обзор Системы

Фокус платформы смещён на **базы знаний**: документы и страницы хранятся в PostgreSQL, чанки векторизуются и складываются в Qdrant, а поиск и RAG-ответы строятся поверх этих данных.

### Что важно знать сейчас

* ✅ Рекомендуемый сценарий — работать через базы знаний (`/api/knowledge/*`) и публичные векторные эндпоинты.
* ✅ Поддерживаются гибридный поиск, векторизация чанков и генеративные ответы на основе найденных документов.
* ❗️Легаси-эндпоинты управления сайтами и страницами больше не доступны.

> ❗️ **Удалённые маршруты**: `/api/search`, `/api/sites*`, `/api/pages*`, `/api/stats`, `/api/webhook/crawl`. Все запросы к ним теперь возвращают ошибку или не регистрируются. Используйте базы знаний или публичный RAG API.
...
- При передаче `"stream": true` (GigaChat включает поток по умолчанию), обязательно добавляйте заголовок `Accept: text/event-stream, application/json`, иначе сервис вернёт обычный JSON и потоковые события не будут отправлены.
