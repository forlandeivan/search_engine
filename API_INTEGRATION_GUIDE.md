...
> `publicApiKey` передаётся в заголовке `X-API-Key`, а `publicId` — в пути (`:publicId`) или в параметре `sitePublicId`.
> Ошибка 401 означает отсутствие заголовка или несоответствие ключа сайту, 404 — неверный `publicId`, коллекция другого workspace или неподключённые провайдеры.
...
**Если получили 403 Forbidden:**

- Бэкенд возвращает 401 JSON-ответ при неверном ключе и 404 при несоответствии `publicId`/коллекции. Ошибку 403 в виде простого текста чаще выдаёт пограничная инфраструктура (корпоративный прокси, WAF, CDN), не пропустившая HTTPS-туннель.
- Признак такой ситуации в `curl` — сообщение `curl: (56) CONNECT tunnel failed, response 403` и отсутствие JSON-ошибки сервера.【f74c91†L8-L15】
- Проверьте: не требуется ли авторизация на прокси, присутствует ли домен в allowlist, не блокирует ли корпоративная фильтрация соединения. После разрешения сетевого доступа запрос проходит и возвращает JSON.
...
#### Пошаговая диагностика 401/403/404

1. **Проверить `publicId` в URL.** Он должен совпадать со значением из настроек сайта. Ошибка в одной букве приводит к 404.
2. **Сверить `collection` в теле запроса.** Имя коллекции чувствительно к регистру и должно существовать в workspace сайта.
3. **Подтвердить действительность `publicApiKey`.** Деактивированные или чужие ключи дают 401. Сформируйте новый ключ в админке и повторите запрос.
4. **Убедиться, что выбранные `embeddingProviderId` и `llmProviderId` подключены к workspace.** Для неподключённых провайдеров сервер вернёт 404.
5. **Посмотреть логи.** При 401/404 сервер пишет, какой именно check провалился (например, «public key disabled»). Если запрос блокирует прокси с 403, проверьте его журналы.

> 💡 Для оперативной проверки можно выполнить `curl -v` с теми же параметрами — в ответе будут видны точные HTTP-заголовки и код, а в логах сервера появится расшифровка ошибки.
